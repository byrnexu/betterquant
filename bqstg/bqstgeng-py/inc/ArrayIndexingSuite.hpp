...
class array_indexing_suite
 ...
 public:
  ...
  static data_type& get_item(Array& arr, index_type i) { return arr[i]; }
  ...
  static size_type size(Array& arr) { return arr.size(); }

  static bool contains(Array& arr, key_type const& key) {
    return std::find(arr.cbegin(), arr.cend(), key) != arr.cend();
  }

  static index_type get_min_index(Array&) { return 0; }

  static index_type get_max_index(Array& arr) { return arr.size(); }
  ...
  static boost::python::object get_slice(Array& arr, index_type from,
                                         index_type to) {
    if (from > to) {
  ...
  static void set_slice(Array& arr, index_type from, index_type to,
                        data_type const& v) {
    if (from > to) {
      return;
  ...
  static void set_slice(Array& arr, index_type from, index_type to, Iter first,
                        Iter last) {
